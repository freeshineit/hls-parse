...
(function () {
  ...
  document.querySelectorAll('#tab-url .sample-btn').forEach(function (btn) {
    btn.addEventListener('click', function () { urlInput.value = this.dataset.url; });
  });

  // ---- Sample texts ----
  document.querySelectorAll('#tab-text .sample-btn').forEach(function (btn) {
    btn.addEventListener('click', function () { textInput.value = SAMPLES[this.dataset.sample]; });
  });
  ...
  function valSpan(val) {
    ...
    if (typeof val === 'string' && /^https?:\/\//.test(val))
      return '<span class="j-uri">' + esc(val) + '</span>';
  ...
  function section(label, data, meta) {
    ...
    if (Array.isArray(data) && data.length > 0 && typeof data[0] === 'object' && !(data[0] instanceof Date)) {
      ...
      data.forEach(function (item, i) {
        ...
        h += '<td class="j-mono"><button style="font-size:.65rem;padding:0 .35rem" onclick="var r=document.getElementById(\'' + rid + '\');r.style.display=r.style.display===\'table-row\'?\'none\':\'table-row\'">+</button></td>';
        h += '</tr>';
  ...
  function render(pl) {
    ...
    if (isMst) {
      // Master playlist
      if (pl.variants && pl.variants.length) html += section('variants', pl.variants, {
        badge: 'VARIANTS', badgeClass: ' j-type-master',
      ...
      if (pl.start) html += section('start', pl.start, { badge: 'START' });
      if (pl.defines && pl.defines.length) html += section('defines', pl.defines, { badge: 'DEFINES' });
      if (pl.segments && pl.segments.length) html += section('segments', pl.segments, {
        badge: 'SEGMENTS',
        cols: ['mediaSequenceNumber', 'uri', 'duration', 'discontinuity', 'gap'],
        rowFmt: function (c, it) { if (c === 'discontinuity') return it.discontinuity || false; if (c === 'gap') return it.gap || false; return it[c]; }
      });
